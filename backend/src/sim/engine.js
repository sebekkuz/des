*** a/backend/src/sim/engine.js
--- b/backend/src/sim/engine.js
@@
...
+    // Struktury modelu
+    this.defs = new Map();     // id -> definicja z modelu
+    this.links = [];           // [{from,to}]
...
+
+    // Stan komponentów
+    this.buffers = new Map();      // id -> {cap, q: number}
+    this.workstations = new Map(); // id -> {busy:boolean, m:number, svcDist:any, inQ:number}
   }
...
+    }
+
+    // zidentyfikuj sinki/bufory/workstations
+    this.sinks.clear();
+    this.buffers.clear();
...
+      // sprawdź, czy jest jakakolwiek ścieżka do SNK
+      const reachesSink = this._hasPathToSink(id);
       if (!reachesSink) {
...
+  // --- RUCH JEDNOSTEK ---
+  // Zdarzenie przybycia z generatora: kierujemy do pierwszego sąsiada
   _onArrival(g) {
...
+  // Wepchnięcie jednostki do komponentu 'id'
+  _pushTo(id) {
...
+        if (b.q < cap) {
+          b.q++;
+          this._tryPushingFromBuffer(id); // może da się wypchnąć dalej
+        } else {
+          // przepełnienie – w MVP po prostu zostaje (możesz zalogować)
+          this.onLog({ level: 'warn', msg: `Buffer ${id} full (q=${b.q}/${cap})`, at: Date.now() });
+        }
+      } else {
+        // Bufor bez stanu? potraktuj jak przelotowo
...
+      if (!ws) return;
+      // dodaj do kolejki wejściowej
+      ws.inQ++;
+      this._tryStartService(id);
+      return;
+    }
+
+    // Domyślnie – przelotowo
+    const next = this._firstNext(id);
+    if (next) this._pushTo(next);
+  }
+
+  // Próba wypychania z bufora do kolejnego komponentu
...
+      // jeśli WS ma wolne miejsce (idle slot) – zdejmij z bufora i wrzuć do WS
...
+    }
+
+    // W innym wypadku – przelot (bez sensownego flow) – zdejmij i przekaż dalej
+    if (b.q > 0) {
+      b.q--;
+      this._pushTo(next);
+    }
+  }
+
+  // Start obsługi w Workstation (jeśli są wolne sloty i czeka kolejka)
...
+  }
+
+  // Zakończenie obsługi – przekaż do kolejnego elementu
+  _onServiceDone(wsId) {
+    const ws = this.workstations.get(wsId);
+    if (ws && ws.busy > 0) ws.busy--;
+    // po zakończeniu spróbuj uruchomić kolejne sztuki
+    this._tryStartService(wsId);
+    // wyślij dalej
+    const next = this._firstNext(wsId);
+    if (next) this._pushTo(next);
+    // jeśli WS poprzedza bufor – spróbuj też wypchnąć z bufora (może zwolniło się miejsce)
